...
If you want to start contributing for `React Event Handler Wrapper Library`, read the documentation below to understand more about the project structure and its directives.
...
It will create a local link of the library named as "react-event-handler-wrapper" (so you can use it on your target project)
...
```bash
yarn link "react-event-handler-wrapper"
...
react-event-handler-wrapper:registry=http://localhost:4873
```

> PS: `Way 01` is the best option because automatic identifies when to download from verdaccio's registry, so if you publish many times you just need to run `yarn` or `npm install` to update the `react-event-handler-wrapper` package (instead of manually ask to download it from verdaccio's registry).
...
```bash
yarn add react-event-handler-wrapper --registry http://localhost:4873
```
